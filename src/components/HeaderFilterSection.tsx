...
const HeaderFilterSection: FC<HeaderFilterSectionProps> = ({
  ...
  const [tabActive, setTabActive] = React.useState("Others");

  useEffect(() => {
    handleSelectedCategories(tabActive);
  }, [tabActive]);
  ...
  return (
    <div className={`flex flex-col relative ${className}`}>
      <p className={"text-3xl font-semibold mb-[30px]"}>What's trending now</p>
        ...
        >
          {categories?.map((item: any, index: number) => (
            <NavItem
              ...
              key={index}
              isActive={tabActive === item.category}
              onClick={() => setTabActive(item.category)}
            >
              {/* <button onClick={() => setTabActive(item.category)}> */}
              {item.category}
              {/* </button> */}
            </NavItem>
          ))}
      ...
      >
        ...
        <TabFilters categories={categories} activeCategory={tabActive} />
      </Transition>
    </div>
  );
};
