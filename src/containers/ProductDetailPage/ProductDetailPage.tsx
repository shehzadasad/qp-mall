...
import { fetchReviews } from "redux/Review/GetReview";
...
import { fetchSimilarProducts } from "redux/HomePage/fetchAllProducts";
...
const ProductDetailPage: FC<ProductDetailPageProps> = ({
  ...
  const similarProducts = useSelector(
    (state: any) => state.products.similarProducts
  );
  const [productData, setProductData] = useReducer((prev: any, next: any) => {
    return { ...prev, ...next };
  }, {});
  const reviewsData = useSelector((state: any) => state.GetReviews.review);
  ...
  const fetchSimilarProductsData = () => {
    dispatch(
      fetchSimilarProducts({
        ...
        similarProductsPayload: {
          ...
          offset: 0,
          category: "Others",
          tags: product?.product_tags?.tags,
        },
      })
    );
  };
      ...
      <div>
        <div className="flex justify-between font-medium text-sm">
          ...
          <p className="text-slate-500">
            a Select your {productAttributes.name.toLowerCase()}
          </p>
        </div>
                ...
                (items: any, index: any) => {
                  return (
                    <div
                      key={index}
                      className={
                        "relative rounded-lg border flex items-center justify-center text-sm sm:text-base uppercase font-semibold select-none overflow-hidden z-0 border-slate-300 dark:border-slate-600 text-slate-200 dark:text-slate-200 cursor-not-allowed lg:px-15 lg:py-2 px-10 py-3"
                      }
                    >
                      {items}
                    </div>
      ...
      <div className="">
        ...
        <h2 className="text-2xl font-semibold flex items-center">
          ...
          <span className="ml-1.5">
            {" "}
            {review?.average_rating}, {review?.num_reviews} Reviews
          </span>
        </h2>
            ...
            {reviews?.map((r: any) => {
              return (
                <ReviewItem
                  name={r?.user_name}
                  avatar={r?.user_image}
                  date={r?.review?.created_at}
                  rating={r?.review?.rating}
                  comments={r?.review?.review_text}
                  reviewImage={r?.review?.review_image}
                />
              );
            })}
          </div>
          {review?.num_reviews > 2 ? (
            <ButtonSecondary
              onClick={() => setIsOpenModalViewAllReviews(true)}
              className="mt-10 border border-slate-300 dark:border-slate-700 "
            >
              Show me all {review?.num_reviews} reviews
            </ButtonSecondary>
          ) : (
            " "
          )}
  ...
  return (
    <div className={`nc-ProductDetailPage ${className}`}>
      ...
      <main className="container mt-5 lg:mt-11">
        <div className="lg:flex">
          ...
          <div className="w-full lg:w-[55%] ">
            ...
            <div className="relative">
              ...
              {renderStatus()}
              {/* META FAVORITES */}
              <LikeButton liked={like} className="absolute right-3 top-3 " />
            </div>
        ...
        {/* DETAIL AND REVIEW */}
        <div className="mt-12 sm:mt-16 space-y-10 sm:space-y-16 pb-10">
          {/* <div className="block xl:hidden">
            <Policy />
          </div> */}
          {/* {renderDetailSection()} */}
          {/* <hr className="border-slate-200 dark:border-slate-700" /> */}
          {review?.num_reviews > 0 ? (
            <>
              <hr className="border-slate-200 dark:border-slate-700" />
              {renderReviews()}
            </>
